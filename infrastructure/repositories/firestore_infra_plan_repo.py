"""Firestore-backed InfrastructurePlanRepository — production implementation for M05."""
...
COLLECTION = "infrastructure_plans"


class FirestoreInfrastructurePlanRepository(FirestoreBase):
    """Implements InfrastructurePlanRepositoryPort using Firestore."""

    # Re-use the same _to_dict/_from_dict logic as the in-memory version
    @staticmethod
    def _to_dict(plan: InfrastructurePlan) -> dict:
        hc, ac, nc, sc, ce = (
            plan.hana_config,
            plan.app_server_config,
            plan.network_config,
            plan.security_config,
    ...
    async def save(self, plan: InfrastructurePlan) -> None:
        await self._doc(COLLECTION, plan.id).set(self._to_dict(plan))
